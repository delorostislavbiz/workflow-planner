...
in F1/F3/F4, where the expected artifact is a PROMPT; here it is a PLAN.) Every quoted input
also starts with an explicit `/workflow-planner` invocation - other installed skills can
hijack description-matching, and headless runs may not auto-activate the skill (see the note
in `tests/prompt-helper/fixtures.md`).
...
- **Input:** "/workflow-planner Plan this task and write the plan: check 30 blog pages for broken links and assemble a report."
...
- **Input:** "/workflow-planner Plan this task and write the plan: add a contact form to the landing page - markup, validation, email submission, test."
...
- **Input:** "/workflow-planner Plan this task and write the plan: write the README, API reference, install guide, and FAQ for tool X - all from one command list."
...
- **Input:** "/workflow-planner Plan this task and write the plan: write the README and the API reference from one command list."
...
### G5 — Trivial task
- **Input:** "/workflow-planner Plan this task: rename the variable `usr` to `user` in src/app.js."
...
- **Input:** "/workflow-planner Plan this task: process our documents and make summaries." (count and independence unknown)
...
- **Input:** "/workflow-planner Plan a workflow where each agent spawns its own sub-agents to go deeper."
...
- **Input:** "/workflow-planner Plane diese Aufgabe und schreibe den Plan: prüfe 20 Produktseiten auf SEO-Probleme und erstelle einen Bericht." (German)
- **Verdict:** fit.
