# Prompt Helper — build a correct workflow prompt (Codex)

The add-on flow of workflow-planner. It helps a user (newcomer-first) turn a fuzzy
idea — or a rough draft — into a **correct task description ("prompt") for a Codex
multi-agent workflow** (flat subagents orchestrated by the main agent). It leads with the
gate ("do you even need a workflow?"), then runs a recipe or an interview, assembles the
prompt, and offers an opt-in inline check. It **never runs the target workflow**.
...
| Input | Route |
|------|-------|
| A task already shaped as a workflow prompt | existing `gate -> plan` (not this flow) |
...
> "A workflow means several subagents working at once or in stages, run by the main agent.
> It pays off when there are same-type chunks to do at once, or a multi-stage flow at
> volume. If it's just steps one after another, a plain linear plan is better."
...
10. **Write-boundaries & environment** — Q: "Which files does each subagent touch?" — why: parallel writes collide. Mini-checklist: which files are read-only; what each subagent may change; who merges/synthesizes; what to do on conflict; whether parallel writes need a git worktree for isolation (see `reference/codex-workflow-runtime.md`); which commands are forbidden. No nested subagents (workers are flat leaves).
...
parallelism / a missing barrier; a shared source without an anchor; a subagent missing
...
Hand off: present the clean prompt; state the next step ("hand it to the Codex planner —
it runs the gate and writes the plan, and on your approval runs the flat subagents; I won't
run anything"); offer to save it to a file.
...
Per-subagent data              (paths / anchor artifact, not "context above")
Verify-by-content, per unit
Autonomy & emergencies         (by risk level)
Acceptance criteria
Write-boundaries & environment (read-only files, ownership, git worktree if needed)
Output
```
...
- **Subagents must change overlapping files** -> checklist item 10 (git worktree / ownership).
- **High-stakes (legal / medical / financial)** -> heightened caution; explicit confirmations; don't over-promise.
- **Asks for nested sub-agents** -> there are none (workers are flat leaves); offer a flat shape, or route to agent-constructor.
