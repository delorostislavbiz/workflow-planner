...
0. **Prompt or not? (entry routing).** First decide what the user brought. A raw idea / rough draft / "help me write a prompt" / "do I need a workflow?" -> go to the **Prompt Helper** (`reference/prompt-helper.md`) first to build the workflow prompt. An already-shaped task or a ready prompt -> skip prompt-building and go straight to step 1. Building the prompt is a separate front stage, not part of planning. **Tie-break (verified live: this routing drifts):** a bare imperative task with no explicit plan request ("check 25 posts and give me a summary") defaults to the Prompt Helper build-path - its artifact is a PROMPT. Only an explicit plan request ("plan this...", "спланируй", "напиши план") routes to the planner (artifact: a PLAN). Name the chosen route in one line when you start and do not drift between the two mid-flow.
1. **Understand the task.** State assumptions up front. If the task is ambiguous and guessing would change the plan, ask a concise clarifying question - within the same 2-question cap (speech rule 4); everything else becomes an explicit assumption.
1.5. **Build the Acceptance Contract (judge).** Before routing or planning, establish what "done" means for the whole task. Read `reference/acceptance-contract.md` and build an Acceptance Contract: observable predicates, fixed before the work starts, including an independent out-of-sample check and a stop condition. Honor the delegation slider - if the user gives criteria, structure and drill them; if the user is tired or says "decide yourself", auto-draft and then get ratification. Trivial tasks: a one-line contract is enough - do not run a full interview. The contract is **frozen at plan approval** - changing it later is a new round with explicit re-ratification, not a silent edit. **Sequencing:** whenever dialogue is possible, ratify the contract as its own question BEFORE writing the plan (a quick "ok?" is enough). Only when the turn cannot wait for an answer (single-shot / non-interactive) embed the draft contract into the plan, label it "draft - not ratified", and make plan approval explicitly cover the contract too. It feeds the rest of the plan: per-step verifies, the whole-task post-verify, and the rubric handed to any verifier subagent - all check **against** this contract, not ad hoc.
2. **Applicability gate.** Read `reference/applicability.md`. Decide whether the task benefits from flat Codex subagents or should stay linear. If borderline, ask 1-2 clarifying questions - **never more than 2 in one turn**; cover every remaining unknown with an explicit assumption ("correct me if wrong") instead of a third question.
3. **Linear path.** If not a fit, use `templates/linear-plan.md` and write `PLAN.md` in the project root. Lint it mechanically: `node <skill-dir>/tools/lint-plan.js <plan>` (the linter ships inside this skill's `tools/`) - fix every ERROR; if node or shell access is unavailable, say so and rely on the template's self-review alone. Show the plan and stop.
4. **Workflow path.**
   - Read `reference/plan-to-codex-workflow.md` and `reference/codex-workflow-runtime.md`.
   - Use `templates/codex-workflow-plan.md` to write an atomic workflow plan. Lint it mechanically before showing it: `node <skill-dir>/tools/lint-plan.js <plan>` - fix every ERROR; if node or shell access is unavailable, say so and rely on the template's self-review alone.
   - Show the plan and wait for explicit approval before running any subagents. **Approval means an explicit yes to THIS question.** A reply that answers something else, only adds corrections, or is ambiguous is NOT approval: apply/acknowledge the corrections and re-ask the approval in one line - never start subagents on inferred consent.
...
Every linear step or workflow atom is one meaningful action with its own verify. Split any step that hides 3+ sub-actions or more than roughly half a day of work. Avoid the opposite mistake too: do not create one subagent per micro-step. A subagent branch should be a coherent bundle of atoms. **The inverse bound holds for trivial tasks: 1-3 steps total.** Fold the checks into each step's verify - the contract check lives in the LAST step's verify, not as its own step; a separate file-existence step or inventory step around a one-line edit is inflation, not rigor.
